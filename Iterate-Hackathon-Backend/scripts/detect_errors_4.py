...
def check_id_consistency(df: pd.DataFrame) -> List[str]:
    ...
    id_columns = ['Barcode', 'Headoffice ID', 'Sale ID']
    consistency_check_columns = {
        'Barcode': ['Product', 'Packsize', 'Trade Price', 'RRP'],
        'Headoffice ID': ['Product', 'Packsize'],
        'Sale ID': []
    }
    
    for id_col in id_columns:
        if id_col in df.columns and id_col in consistency_check_columns:
            check_cols = consistency_check_columns[id_col]
            for check_col in check_cols:
                if check_col in df.columns:
                    grouped = df.groupby(id_col)[check_col].nunique()
                    inconsistent_ids = grouped[grouped > 1].index.tolist()[:3]
                    for id_val in inconsistent_ids:
                        subset = df[df[id_col] == id_val]
                        different_values = subset[check_col].unique().tolist()[:5]
                        example_rows = subset.index.tolist()[:3]
                        errors.append(f"ERROR: Inconsistent data for {id_col} '{id_val}': multiple {check_col} values found {different_values}. Example rows: {example_rows}.")
...
def check_business_logic(df: pd.DataFrame) -> List[str]:
    ...
    if 'Turnover' in df.columns and 'Turnover ex VAT' in df.columns:
        ...
        if invalid_mask.any():
            invalid_indices = df[invalid_mask].index.tolist()[:3]
            errors.append(f"WARNING: Turnover is less than Turnover ex VAT in some rows. Example rows: {invalid_indices}.")
    ...
    if 'Qty Sold' in df.columns and 'Turnover' in df.columns:
        ...
        if zero_qty_positive_turnover.any():
            ...
            errors.append(f"WARNING: Zero quantity sold but positive turnover. Example rows: {invalid_indices}.")
...
def main():
    ...
    summary_info = summarize_dataset(df)
